...
def build_function_chunk_prompt(
    ...
    return (
        ...
        "4. FRONTEND EXCLUSION RULE (CRITICAL): If this is a React/Browser/Frontend environment (e.g., js, jsx, ts, tsx files defining UI components), it CANNOT have 'IDOR / Broken Access Control', 'Mass Assignment', 'SQL Injection', 'Command Injection', 'Path Traversal', or 'Missing Authentication'. The frontend is an untrusted client and cannot enforce backend boundaries.\n"
        "5. FRONTEND BUSINESS LOGIC RULE: For frontend code, look for 'Optimistic UI De-Sync' (firing a fetch/axios call and updating local UI state without checking if the response was successful via res.ok or a robust try/catch). Also look for raw PII/tokens stored improperly in LocalStorage. Classify these as 'Business Logic Flaw'.\n"
        "6. HTTP METHOD CONSISTENCY RULE: Treat ALL state-modifying HTTP requests (POST, PUT, PATCH, DELETE) with identical severity thresholds. Do not exhibit lexical bias towards the word 'DELETE' while ignoring identical dynamic interpolations in 'PUT' or 'POST'.\n"
        "7. VULNERABILITY HIERARCHY: Injections (SQLi, Command, Path) always take precedence over IDOR. Look for Injections first.\n\n"
        "8. PLAINTEXT PASSWORD: If a function compares passwords with == instead of using a hashing library (bcrypt, hashlib with salt, etc.), classify as 'Plaintext Password'. This is HIGH severity.\n"
        "9. MISSING AUTHENTICATION: If a backend route handler (Flask route, FastAPI endpoint, Express route) lacks an auth check/decorator AND accesses user data/admin info, classify as 'Missing Authentication'. HIGH severity.\n"
        "10. MASS ASSIGNMENT: If a backend function accepts arbitrary request body fields and passes them directly to a model/ORM constructor or update() without field allowlisting, classify as 'Mass Assignment'.\n"
        "11. INPUT VALIDATION FAILURE: If a function uses a complex regex with potential for catastrophic backtracking (ReDoS), or fails to validate/sanitize types properly, classify as 'Input Validation Failure'.\n\n"
        ...
        "   CRITICAL: DO NOT list safe utility functions, routing decorators, or response wrappers as sinks. "
        "Functions like `jsonify(...)`, `request.get_json(...)`, `print(...)`, `len(...)`, `return ...` are NEVER sinks. "
        "Only list the specific function call that actually executes the exploit (e.g., `user.update(...)` for Mass Assignment).\n\n"
