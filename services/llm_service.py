...
def build_function_chunk_prompt(
    ...
    return (
        ...
        "C. CROSS-FILE & UTILITY METHODS (CRITICAL): If a function is a generic database/system wrapper (e.g. `def run_query(self, query): db.execute(query)`), you MUST flag it as vulnerable! Even if there is no HTTP request visible in the snippet, assume `query` is tainted from a higher-level API route. We rely on you to flag these deep sinks so our Call Graph Resolver can trace them backwards. DO NOT SKIP utility functions just because they lack web context!\n"
